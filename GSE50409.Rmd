...
for (i in 1:n){
  cat(i, "\n")
  expected <- cbind(expected, computeFit(permute = TRUE)$P.Value < 0.05)
  #permutedFit <- computeFit(permute = TRUE)$P.Value < 0.05
  #expected[i] <- mean(permutedFit$P.Value < 0.05)
}

write.csv(imputed$data, "GSE50409_matrix.csv")
write.csv(expected, "GSE50409_expected.csv")
write.csv(observed, "GSE50409_observed.csv")
