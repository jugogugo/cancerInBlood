...
ncol(normalizedMatrix_without_1stoutliers), replace = TRUE)
...
for (iteration in 1:100) {
  ...
  fit <- lmFit(normalizedMatrix_without_1stoutliers, model)
  fit <- eBayes(fit)
...
Final analysis
```{r, include=TRUE, echo=TRUE}
age <- rnorm(ncol(normalizedMatrix_without_1stoutliers), mean=60)
